...
仓库中的基坑 OBJ 由多个相互贴合和局部重叠的闭合零件组成。原模型在
`x≈1.5、z=-29` 处存在约 `9.33e-7` 宽的数值伪薄带，直接四面体化得到
的最差单元 `mean-ratio` 只有 `4.541e-5`。程序在零件布尔并集前统一确认
为建模误差的近重合坐标面，再重新生成单区域实体和四面体网格。

最终结果包含 6958 个节点和 20552 个四面体：

- 最小 `mean-ratio`：`0.33952`，1% 分位数：`0.43882`，中位数：`0.74903`；
- 反转、零体积、重复四面体和体边界错配均为 `0`；
- 低于质量门槛 `0.05` 的单元由 4 个降为 `0`；
- 相对几何偏差为 `3.90e-13`，体积相对误差为 `4.30e-16`。

下图是从最终 `.msh` 体网格提取的实际外边界，共 11474 个三角面。颜色表示
相邻四面体的 `mean-ratio`，不是只对修复后的 OBJ 做表面渲染。

![基坑最终 CAE 外边界网格](docs/validation/pit_cae_boundary_mesh.png)

下图是在 `y=7` 处对四面体网格进行的精确剖切，左下角放大了原数值伪薄带
所在区域，可以看到该区域没有继续生成近零体积的薄楔单元。

![基坑内部四面体网格剖面](docs/validation/pit_cae_tetra_section.png)
